...
function am_add_javascript( ) {
    ...
    if( !is_admin() ) {
        //$am_files = array('includes/js/map/leaflet.js', 'includes/js/map/leaflet.markercluster-src.js', 'includes/js/map/leaflet.spin.js', 'includes/js/map/spin.min.js', 'includes/js/jquery.main.js');
        $am_files = array('includes/js/jquery.main.js', 'includes/js/slick/slick.min.js', 'includes/js/general.js');
        ...
        $am_links = array('https://cdnjs.cloudflare.com/ajax/libs/gsap/3.6.1/gsap.min.js');
...
function am_add_css( ) {
    ...
    $am_files = array(
        ...
        'css/slick/slick.min.css',
		'css/slick/slick-theme.min.css',
        'style.css', 
        'style-wp.css',
    );
    foreach($am_files as $am_file){
    ...
    // external CSS
    $am_links = array('https://fonts.googleapis.com/css2?family=Cormorant:ital,wght@0,400;0,600;1,400;1,600&family=Montserrat:wght@400;600;800&family=Work+Sans:wght@300;400&display=swap'); 
    foreach($am_links as $am_link){
        wp_enqueue_style('am_'.sanitize_title($am_link),$am_link,array(), null);
    }
}
...
function am_login_logo(){  ?>
    <style type="text/css">
        body.login div#login h1 a {
			...
			cursor: pointer;
			text-indent: -9999em;
			background: url(<?php echo get_bloginfo( 'template_directory' ) ?>/images/logo-black.svg) no-repeat;
